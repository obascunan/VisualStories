...
function circleBackground() {
  ...
  for (var x = 0; x < width; x += xDistance) {
    for (var y = 0; y < height; y += xDistance) {
      ...
      fill(x / width * 60 + sin(millis() / 1000 + y / height) * 30 + 10, 90);
...
function drawText() {
 ...
 // fill(random(10));
  text("P    o  e  m  s        o     f          t   e  m  p  o  r  a  r   y        l    a    n   d  ", width / 2, height / 2);
} 
...
function squareGrid() {
  ...
  for (var x = 0; x < width; x += xDistance) {
    for (var y = 0; y < width; y += xDistance) {
      ...
      rect(x+random(2), y+20+random(2), r, r);
      //    line(0, y, windowWidth, y);
    }
  }
}
...
function drawRainLine() {
  ...
  var y = floor( height / 10);
  stroke(50, 100, random(120, 255));
  line(x, 0, x + 200, 250);
}
...
var Particle = function(position) {
  ...
  var angle = PI * random(0, 1);

var __x = width * random (0,1);
  this.position = createVector(__x, height /8+cos(__x*PI/width+millis()/2000)*30);
//this.position = createVector(width / 2 + cos(angle) * 300, height / 2);

  if (random() > 0.99) {
    this.lifespan = 120.0;
    this.velocity = createVector(0, sin(angle/2) * 4.5/2);
  } else {
 ...
 // if (random() < 0.8) {
  ...
  //} else {
    this.mainColor = color(60 * cos(angle) + 175, 180, 50 * sin(angle) + 205);
  //}
...
Particle.prototype.display = function() {
  ...
  var toColor = lerpColor(color(255), this.mainColor, (120-this.lifespan)/60 );
 // fill(red(this.mainColor), green(this.mainColor), blue(this.mainColor), sin(this.lifespan / 90 * PI) * 105);
  //toColor.a 
  //fill(toColor, 10);//sin(this.lifespan / 120 * PI) * 5);
  //  fill(120, this.lifespan * 2);
  //  fill(127, this.lifespan/50);
 fill(red(toColor), green(toColor), blue(toColor), sin(this.lifespan / 90 * PI) * 105);
  ...
  rect(this.position.x, this.position.y, size, size*3);
};
